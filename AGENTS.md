...
- `enable_docs` registers `/openapi.json`, `/docs` and `/redoc` with `include_in_schema=false`, which is why turning documentation on does not change the document. Any route that serves the app's own metadata should carry that flag.
